...
#R1(i,j) in E: Lambda_max >= sum{(s,r) in R}Lambda[i,j,s,d]
    ...
    for i in E:
        lhs = Lambda_max
        rhs = grb.quicksum([Lambda[i[0],i[1],j[0],j[1]] for j in R]) + LambdaT[i[0]][i[1]]    
...
if __name__ == '__main__':
    pasta = "C:\\Users\\Artur Alvarenga\\Documents\\GitHub\\RACO\\testeC\\"
    ...
    resp = "C:\\Users\\Artur Alvarenga\\Documents\\GitHub\\RACO\\ResultadosCluster\\resultados30.txt"
    for Na, arq in enumerate(caminhos):
        ...
        for i in range(len(clt)):
            Lbd, status = Mrwae(clt[i], Inst, lamb)
